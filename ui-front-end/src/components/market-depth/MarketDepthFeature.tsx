...
export const MarketDepthFeature = () => {
    // instead of fetching real data (useMarketDepthData) we will use the testData for now
//     const data = testData;

  // fetching the real market depth data using the hook
  const data = useMarketDepthData(schemas.prices);
   
//   return <MarketDepthPanel data={testData} />;
  // Render the MarketDepthPanel once the data is fetched
    return <MarketDepthPanel data={marketDepthData} />;
